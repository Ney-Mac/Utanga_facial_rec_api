...
async def preparar_turma_total(db: Session):
    ...
    # Sorteia um atraso superior a 40 min e no máximo 2 horas
    atraso_minutos = random.randint(41, 120)
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code != 200:
            raise HTTPException(
                ...
                detail=f"Erro ao criar a turma total: {response.text}"
            )
