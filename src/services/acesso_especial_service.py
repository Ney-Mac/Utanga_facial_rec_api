...
async def solicitar_acesso(id_usuario: str, id_turma_destino: str, db: Session):
    ...
    if cadeira and turma and solicitante:
        ...
        enviar_mensagem("Recebei um pedidio de acesso especial. Verifique.")
...
async def responder_pedido(id_solicitacao: int, aceitar: bool, db: Session):
    solicitacao_acesso = db.query(TableSolicitacaoAcessoEspecial).filter_by(id=id_solicitacao).first()
    ...
    resposta = nova_solicitacao.aceitar_acesso_especial(db) if aceitar else nova_solicitacao.negar_acesso_especial(db)
...
async def listar_solicitacoes_acesso_filtrado(db: Session, id_turma: str = None, id_cadeira: str = None):
    ...
    if id_turma:
        query = query.filter(TableSolicitacaoAcessoEspecial.id_turma == id_turma)
    if id_cadeira:
        query = query.filter(TableSolicitacaoAcessoEspecial.id_cadeira == id_cadeira)
